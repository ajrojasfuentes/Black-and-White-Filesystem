...
/**
 * Logging macros for BWFS
 */
#define BWFS_LOG_INFO(fmt, ...) \
    fprintf(stdout, "[BWFS INFO] " fmt "\n", ##__VA_ARGS__)
#define BWFS_LOG_ERROR(fmt, ...) \
    fprintf(stderr, "[BWFS ERROR] " fmt " (%s)\n", ##__VA_ARGS__, strerror(errno))

/**
 * Create an empty block file (PNG container) initialized to zeros.
...
/**
 * Write raw data to a block file.
 * @param fs_dir   Filesystem directory
 * @param block_id Block index
...
/**
 * Read raw data from a block file.
 * @param fs_dir   Filesystem directory
 * @param block_id Block index
